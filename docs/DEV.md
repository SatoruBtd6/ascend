...
`node scripts/aura-p3b-perf.mjs --base http://localhost:5173` (dev server must be running) reports loop and moment frame cost per moment aura at 4x CPU, then a worst-case stress: 10 auras at board-32 with all moments forced simultaneously — p95 must stay under 16 ms. `--auras a,b` picks the per-aura list, `--stress-only` skips the per-aura section, and `--stress a,b,...` overrides the fixed stress set. `node scripts/aura-p3b-shots.mjs <auras> <base>` renders loop/moment-phase screenshots at production z-order into `docs/baselines/ascended-7h/`.

7i worn-piece tools: `node scripts/aura-7i-p3-shots.mjs` captures Ledger across all 14 figures plus photo-76/board-32 ring shots into `docs/baselines/ascended-7i/` and reports per-edge margins, including a cloak-isolated margin (spec stripped to robe+emblems so the backdrop gradient can't pollute it). `node scripts/aura-7i-p3-lazyload.mjs` proves robe-ledger.webp and mask-ledger.webp fetch only on ledger mount. `node scripts/aura-7i-p1-unchanged.mjs <beforeBase> <afterBase>` pixel-diffs every aura between two dev servers and compares flashTimes; the expected-diff whitelist covers auras intentionally respecced during 7i. `node scripts/aura-7i-perf-diff.mjs capture|compare <file> [--base url]` snapshots seeded board-32 pixels per stress aura for optimization proofs.
