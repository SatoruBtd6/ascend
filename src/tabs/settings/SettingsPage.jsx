...
export function SettingsPage({ s, setS, onBack, party, setParty, openTool, saveDiag }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {diagOn && (
        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={async () => { try { await navigator.clipboard.writeText(D.formatDump({ version: APP_VERSION, sw: "ascend-v7c.1" })); setDiagCopied(true); } catch (e) { /* clipboard blocked */ } }} className="ghost py-3 text-sm font-bold">{diagCopied ? "Copied" : "Copy diagnostic log"}</button>
